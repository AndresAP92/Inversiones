...
// Set of technical alerts produced by periodic analysis (RSI, news
// sentiment, etc.). These alerts are stored in a Set to avoid
// duplicates and are merged with other alerts when displayed.
let technicalAlerts = new Set();
...
//
// USE_BULK_QUOTES: when true, updateRealTimePrices will fetch quotes for
// all symbols at once using the MCP server's REALTIME_BULK_QUOTES tool.
// This dramatically reduces the number of API calls and speeds up
// updates. When false, the function falls back to fetching prices one
// ticker at a time.
const USE_BULK_QUOTES = true;

// USE_TECH_ANALYSIS: enable or disable hourly technical analysis. When
// enabled the dashboard will periodically compute RSI values and fetch
// news sentiment for your positions and watch list. The results feed
// into the alert system. Note that each technical indicator call
// consumes API quota.
const USE_TECH_ANALYSIS = true;

// TECH_ANALYSIS_INTERVAL: number of milliseconds between technical
// analysis cycles. Defaults to one hour. Adjust this value to fit
// your needs.
const TECH_ANALYSIS_INTERVAL = 60 * 60 * 1000;
...
/**
 * Fetch real-time prices for multiple tickers in one call using the MCP
 * REALTIME_BULK_QUOTES tool. Returns an object mapping each ticker to its
 * latest price. If the call fails or a ticker is missing from the
 * response, that ticker will be omitted from the returned map.
 *
...
async function fetchBulkQuotes(tickers) {
    ...
    // Ensure unique uppercase tickers and truncate to 100 symbols
    const symbols = [...new Set(tickers.map(t => t.trim().toUpperCase()))].slice(0, 100);
    const priceMap = {};
    try {
        // If using MCP provider, construct JSON-RPC payload
        if (REALTIME_PROVIDER === 'alphavantage_mcp') {
            const mcpUrl = `https://mcp.alphavantage.co/mcp?apikey=${API_KEY}`;
            const payload = {
                jsonrpc: '2.0',
                id: Date.now(),
            ...
            if (results && Array.isArray(results)) {
                results.forEach(item => {
                    const symbol = item?.symbol || item?.ticker || item?.Symbol;
                    const priceStr = item?.price || item?.Price || item?.value;
                    if (symbol && priceStr) {
                        const price = parseFloat(priceStr);
                        if (!isNaN(price)) priceMap[symbol.toUpperCase()] = price;
                    }
                });
            }
        } else {
            // Fallback: sequentially fetch using fetchLatestPrice
            for (const t of symbols) {
                const p = await fetchLatestPrice(t);
...
/**
 * Perform periodic technical analysis on portfolio and watch list tickers.
 * This function fetches RSI indicators and news sentiment for each
 * relevant ticker and generates alerts based on standard thresholds.
 */
async function checkTechnicalAnalysis() {
    if (!USE_TECH_ANALYSIS) return;
    if (!API_KEY || API_KEY === 'YOUR_API_KEY') return;
    // Combine portfolio and watcher tickers
    const tickers = [...new Set([...transactions.map(t => t.indice), ...watchers])];
    for (const ticker of tickers) {
        try {
            // Fetch RSI (14-period, daily)
            const rsiUrl = `https://www.alphavantage.co/query?function=RSI&symbol=${encodeURIComponent(ticker)}&interval=daily&time_period=14&series_type=close&apikey=${API_KEY}`;
            ...
            if (rsiSeries) {
                ...
                const rsiVal = parseFloat(rsiSeries[latestDate]['RSI']);
                if (!isNaN(rsiVal)) {
                    if (rsiVal >= 70) {
                        technicalAlerts.add(`${ticker}: RSI ${rsiVal.toFixed(2)} indica sobrecompra. Podría corregir pronto.`);
                    }
                    if (rsiVal <= 30) {
                        technicalAlerts.add(`${ticker}: RSI ${rsiVal.toFixed(2)} indica sobreventa. Podría recuperarse.`);
                    }
                }
            }
            // Fetch news sentiment
            const newsUrl = `https://www.alphavantage.co/query?function=NEWS_SENTIMENT&tickers=${encodeURIComponent(ticker)}&limit=10&apikey=${API_KEY}`;
            ...
            if (feed && Array.isArray(feed) && feed.length > 0) {
                ...
                if (sentiment === 'Positive') {
                    technicalAlerts.add(`${ticker}: Noticias recientes tienen sentimiento positivo. Posible oportunidad de compra.`);
                } else if (sentiment === 'Negative') {
                    technicalAlerts.add(`${ticker}: Noticias recientes tienen sentimiento negativo. Precaución.`);
                }
            }
        } catch (err) {
            console.error('Technical analysis error', ticker, err);
        }
        // Respect rate limits: pause between indicator and news calls
        await new Promise(r => setTimeout(r, 12000));
    }
    // Merge technical alerts into the displayed list
    generateAlerts();
}
...
async function updateRealTimePrices() {
    ...
    try {
        ...
        let priceMap = {};
        if (USE_BULK_QUOTES && REALTIME_PROVIDER === 'alphavantage_mcp') {
            // Attempt to fetch all quotes at once using MCP bulk quotes
            priceMap = await fetchBulkQuotes(tickers);
        }
        // Fallback: fetch individually any tickers not resolved by bulk call
        const unresolved = tickers.filter(t => priceMap[t] === undefined);
        for (const ticker of unresolved) {
            const price = await fetchLatestPrice(ticker);
            if (price !== null) {
                priceMap[ticker] = price;
            }
            // Delay between individual calls to avoid rate limits
            if (REALTIME_PROVIDER === 'alphavantage') {
                await new Promise(r => setTimeout(r, 12000));
            } else {
...
// Kick off periodic technical analysis if enabled. This will run
// independently of price updates and may generate additional alerts
// based on RSI values and news sentiment. See checkTechnicalAnalysis().
if (USE_TECH_ANALYSIS) {
    setInterval(() => {
        if (API_KEY && API_KEY !== 'YOUR_API_KEY') {
            checkTechnicalAnalysis();
        }
    }, TECH_ANALYSIS_INTERVAL);
    // Perform an initial analysis shortly after page load
    setTimeout(() => {
        if (API_KEY && API_KEY !== 'YOUR_API_KEY') {
            checkTechnicalAnalysis();
        }
...
function renderWatchersTable() {
    ...
    // Clear existing rows
    tbody.innerHTML = '';
    // Populate watchers list without DataTables. Using plain HTML reduces dependencies
    ...
    // Rebind remove actions on the plain table
